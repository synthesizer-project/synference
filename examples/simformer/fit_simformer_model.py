...
fitter = Simformer_Fitter.load_saved_model(model_name='simformer_v2_initial',
...
cat_path = '/cosma7/data/dp276/dc-harv3/work/catalogs/JADES-DR3-GS_MASTER_Sel-F277W+F356W+F444W_v13.fits'
...
mask = (cat_sel['Austin+25_EAZY_fsps_larson_zfree_0.32as'] == True) # & (cat_sel['5.60<z<6.50_EAZY_fsps_larson_zfree_0.32as'] == True)
...
bands = [band.split('.')[-1] for band in new_band_names if not (band.startswith('unc_') or band == 'redshift')]
new_table = Table()
...
post_tab = fitter.fit_catalogue(new_table, columns_to_feature_names=conversion_dict, flux_units="AB", return_feature_array=False, check_out_of_distribution=False)

post_tab.write('/cosma7/data/dp276/dc-harv3/work/catalogs/JADES-DR3-GS_MASTER_Sel-F277W+F356W+F444W_v13_fitted_simformer.fits')
