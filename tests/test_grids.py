...
from sbifitter import (GalaxyBasis, CombinedBasis, draw_from_hypercube, generate_sfh_basis, 
                        calculate_muv, SBI_Fitter)
...
def test_grid():
    if not os.path.exists(f"{test_dir}/test_grids/test_grid.hdf5"):
        subprocess.run(["synthesizer-download", "--test-grids", "--destination", f'{test_dir}/test_grids/'], check=True)
...
@pytest.fixture
def lhc_basis_params(test_grid, mock_emission_model, mock_instrument, lhc_prior, lhc_grid, test_zmet, test_sfh, n_params=1e2):
    ...
    with h5py.File(hfile, 'r') as f:
        ...
        if expected_attrs is not None:
            for key, attrs in expected_attrs.items():
                ...
                assert np.all(f.attrs[key] == attrs), f"Attribute '{key}' does not match expected value."
...
class TestGalaxyBasis:
    ...
    def test_create_galaxy(self, grid_basis_params, simple_sfh, simple_zdist):
        ...
        assert galaxy.stars.initial_mass.value == 1e9, f'Expected initial mass to be 1e9 Msun, got {galaxy.stars.initial_mass}'
...
class TestCombinedBasis:
    ...
    def combined_grid_basis_params(self, grid_basis_params):
        ...
        return {
            "bases": [basis1, basis2],
            "total_stellar_masses": unyt_array([1e9] * len(grid_basis_params["redshifts"]), Msun),
    ...
    def combined_lhc_basis_params(self, lhc_basis_params):
        ...
        return {
            "bases": [basis1, basis2],
            "total_stellar_masses": unyt_array([1e9] * len(lhc_basis_params["redshifts"]), Msun),
            "redshifts": lhc_basis_params["redshifts"],
            "base_emission_model_keys": ["total", "total"],
            "combination_weights": np.array([np.array([i, 1-i]) for i in np.random.uniform(0, 1, size=len(lhc_basis_params["redshifts"]))]),
    ...
    def test_create_grid(self, combined_grid_basis_params):
        ...
        expected_keys = ['Grid/Parameters', 'Grid/Photometry', 'Grid/SupplementaryParameters']
...
class TestSBIFitter:
    ...
    def test_init_sbifitter_from_grid(self, test_sbi_grid):
        ...
        assert fitter.grid_path == test_sbi_grid, "SBIFitter did not initialize with the correct grid file."
    ...
    def test_sbifitter_feature_array_creation(self, test_sbi_grid):
        ...
        assert fitter.has_features, "SBIFitter did not create a feature array from the raw photometry."
        assert len(fitter.simple_fitted_parameter_names) > 0, "SBIFitter simple fitted parameter names are empty after feature array creation."
        assert np.shape(fitter.feature_array)[0] == len(fitter.fitted_parameter_array), "SBIFitter feature array shape does not match the expected shape."
        ...
        assert 'redshift' in fitter.feature_names, f"Redshift not found in simple fitted parameter names after adding as extra feature, got {fitter.simple_fitted_parameter_names}"
        assert 'log_mass' in fitter.feature_names, f"Mass not found in simple fitted parameter names after adding as extra feature, got {fitter.simple_fitted_parameter_names}"
        ...
        fitter.create_feature_array_from_raw_photometry(scatter_fluxes=3, depths=depths, include_errors_in_feature_array=True)
        
        # Test including missing bands randomly
        fitter.create_feature_array_from_raw_photometry(
            simulate_missing_fluxes=True,
            missing_flux_fraction=0.3)
        
        # Test including a specific variety of missing bands
        missing_flux_options = np.random.randint(0, 1, (5, len(fitter.raw_photometry_names))).astype(bool)
        fitter.create_feature_array_from_raw_photometry(
            simulate_missing_fluxes=True,
        ...
        fitter.create_feature_array_from_raw_photometry(photometry_to_remove=[fitter.raw_photometry_names[0]])

        # Test removing a feature that does not exist
        with pytest.raises(AssertionError, match="photometry_to_remove must be a list of filter names to remove."):
