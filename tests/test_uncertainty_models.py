...
def test_depth_model_apply_noise():
    ...
    assert np.mean(noise) == pytest.approx(0.0, abs=1e-9)
...
def test_asinh_model_initialization(mock_data):
    ...
    model = AsinhEmpiricalUncertaintyModel(
        observed_phot_jy=fluxes_jy,
        observed_phot_errors_jy=errors_jy
    )
...
def test_asinh_model_low_snr_and_negative_flux(mock_data, temp_dir):
    """
    Tests that the AsinhEmpiricalUncertaintyModel correctly handles
    low SNR data, including negative fluxes, without crashing.
    """
...
def test_apply_noise_statistical_properties(mock_data):
    ...
    assert np.mean(noise) == pytest.approx(0.0, abs=0.02)
...
def test_general_model_apply_scalings(mock_data):
    """
    Tests that the GeneralEmpiricalUncertaintyModel's apply_scalings method
    correctly applies deterministic transformations (unit conversion, SNR cuts)
    without adding random noise.
    """
