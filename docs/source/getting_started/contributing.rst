...
We also have a dedicated slack group for updates and general help with synthesizer and synference. All are welcome to join; please email <chris dot lovell dot astro at gmail dot com> for a sign up link.
...
The `published documentation <https://synthesizer-project.github.io/synference/>`_ reflects the current distribution available on PyPI. If you would like to see the current development version in your branch or the main branch, you will have to build the documentation locally. To do so, navigate to the ``docs`` directory and run:
