...
from ltu_ili_testing import (generate_sfh_basis, SBI_Fitter,
...
# Metallicity
log_zmet = (-3, -1.4) # Metallicity in absolute log scale, i.e. log10(Z/Zsun) where Zsun = 0.02
# ---------------------------------------------------------------
...
combined_basis.process_bases(overwrite=True, mUV=(calculate_muv, cosmo), n_proc=6, verbose=False, batch_size=batch_size)
...
# Initialize SBI 
empirical_model_fitter = SBI_Fitter.init_from_hdf5(hdf5_path=f'grid_{name}.hdf5', model_name=f'sbi_{name}')
...
observed_data_vector = [30.2, 28.7, 28.5, 28.0, 27.5, 27.3, 26.5, 26.2] # Example observed data vector in magnitudes
