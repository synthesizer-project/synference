...
from unyt import Myr, erg, Hz, s, uJy, Jy, yr, unyt_array, unyt_quantity, Msun, Unit
    ...
    def _normalize(self, fluxes, method=None, norm_unit="ABmag", add_norm_pos=-1):
        ...
        if self.min_flux_unit is not None:
            if isinstance(self.min_flux_unit, Unit):
                min_flux = unyt_array(self.min_flux, self.min_flux_unit).to(self.out_flux_unit).value
...
# --- Example Usage (assuming your setup code for GalaxySimulator runs first) ---
if __name__ == '__main__':
    # This block is for demonstration.
    # You need to ensure GalaxySimulator, its dependencies, and run_simulator are defined.
    
    # --- Your provided setup (slightly adapted for this example) ---
    # Make sure all these imports and definitions are available:
   
    
    # Placeholder for SFH and ZDist model classes (replace with your actual classes)
    ...
    # Copied from your code (ensure these are correctly set up in your actual script)
    ...
    filter_codes_list = ["JWST/NIRCam.F090W","JWST/NIRCam.F115W","JWST/NIRCam.F150W","JWST/NIRCam.F162M",
                    "JWST/NIRCam.F182M","JWST/NIRCam.F200W","JWST/NIRCam.F210M","JWST/NIRCam.F250M",
                    "JWST/NIRCam.F277W","JWST/NIRCam.F300M","JWST/NIRCam.F335M","JWST/NIRCam.F356W",
                    "JWST/NIRCam.F410M","JWST/NIRCam.F444W"] # From your code

    priors_ranges_dict = {
        'redshift': (5.0, 10.0), 'log_mass': (7.0, 11.0), 'log10metallicity': (-3.0, -1.3),
        'tau_v': (0.0, 2), 'peak_age': (0.0, 500.0), # Ensure float for peak_age if used with torch.tensor(float(low))
    ...
    galaxy_task = GalaxyPhotometryTask(
        prior_dict=priors_ranges_dict,
        param_names_ordered=inputs_list,
        run_simulator_fn=run_simulator_glob, # Your function to run the simulator
        num_filters=len(filter_codes_list),
        backend="jax" # Or "torch" if you prefer outputs in that format initially
    )
    ...
    # (Optional) Generate validation data if your train_config.val_split is 0 or you want explicit val set
    ...
    rng_seed_for_training = 0 # Or any seed you prefer
