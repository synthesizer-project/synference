...
class GalaxyBasis:
    ...
    def create_galaxies(
        self, stellar_masses: Union[unyt_array, unyt_quantity, None] = None, base_mass: unyt_quantity = 1e9 * Msun
    ) -> List[Type[Galaxy]]:
        """
    ...
    def create_matched_galaxies(self, base_mass: unyt_quantity = 1e9 * Msun) -> List[Type[Galaxy]]:
        """
    ...
    def process_galaxies(
        ...
        **extra_analysis_functions,
        ...
        if save:
            ...
            pipe_fullpath = fullpath.replace(".hdf5", "_0.hdf5")
...
class CombinedBasis:
    ...
    def create_full_grid_old(
        ...
        assert len(self.redshifts) == ngal, (
            f"Redshift array length {len(self.redshifts)} does not match number of galaxies {ngal}. Cannot combine bases with different number of galaxies."
        )
        assert len(self.total_stellar_masses) == ngal, (
            f"Mass array length {len(self.total_stellar_masses)} does not match number of galaxies {ngal}. Cannot combine bases with different number of galaxies."
        )
        assert len(self.combination_weights) == ngal, (
            f"Combination weights array length {len(self.combination_weights)} does not match number of galaxies {ngal}. Cannot combine bases with different number of galaxies."
        )
        ...
        for pos, redshift in enumerate(self.redshifts):
            ...
            for i, base in enumerate(self.bases):
                ...
                if len(mass) == 0:
                    raise ValueError(
                        f"No galaxies found for redshift {redshift} in base {base.model_name}. Check your redshift array."
                    )
