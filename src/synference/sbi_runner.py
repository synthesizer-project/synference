        ...
        for i in trange(grid.shape[1], desc=desc):
            ...
            if callable(method):
                ...
                if center is None or width is None:
                    raise ValueError("'tophat' norm requires 'center' and 'width' in flux_norm_parameters")
                ...
                if filter_obj is None:
                    raise ValueError("'bandpass' norm requires a 'filter' object in flux_norm_parameters")
                ...
                try:
                    ...
                    raise ValueError("Filter object must have 'wavelength' (unyt_array) and 'transmission' (array) attributes")

                # Interpolate filter transmission to spectrum wavelengths
                interp_trans = np.interp(wavs_um, filter_wav, filter_transmission, left=0.0, right=0.0)
                
                # Calculate mean flux in band: integral(F_lambda * T_lambda * d_lambda) / integral(T_lambda * d_lambda)
            ...
            if norm_factor != 0:
                ...
                logger.warning(f"Normalization factor is zero for spectrum {i} ({desc}). Setting flux to zero.")
                grid[:, i] = 0.0
